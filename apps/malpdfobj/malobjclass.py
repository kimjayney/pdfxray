...
class jPdf():
	...
	def dump_data(self, raw_json):
		...
		self._tags = self.shallow_diver(raw_json,"tags")
...
#		if self._stream_decoded_stream != None:
#			self.set_stream_decoded_stream(self.decoder_cleaner(self._stream_decoded_stream))
#		if self._stream_encoded_stream != None:
#			self.set_stream_encoded_stream(self.decoder_cleaner(self._stream_encoded_stream))
		
	def decoder_cleaner(self,magic):
		if self.is_ascii(magic):
			return magic
		else:
			try:
				out = magic.decode("utf-16")
				try:
